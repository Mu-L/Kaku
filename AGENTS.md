...
- Kaku writes nothing into the user's agent tooling (`~/.claude`, `~/.codex`, hooks, settings); agent-facing help goes into documentation only. The one shell-config write is the `kaku init` source line in `.zshrc` (`assets/shell-integration/setup_zsh.sh`, checked by `doctor.rs`), and `reset.rs` must keep every user-owned line around it.
